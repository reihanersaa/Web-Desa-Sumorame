  ...
  formProduk.addEventListener("submit", (e) => {
    ...
    reader.onload = async function (event) {
      try {
        ...
        if (!response.ok) throw new Error(result.message || "Produk gagal diajukan.");
        formProduk.reset();
        tutupModalProduk();
        Swal.fire({
          icon: "success",
        ...
        Swal.fire({ icon: "error", title: "Pengajuan gagal", text: error.message, confirmButtonColor: "#166534" });
      }
    };
...
async function renderProduk() {
  ...
  try {
    ...
    container.querySelectorAll(".produk-item:not(.produk-dynamic)").forEach((item) => item.remove());
...
function updateTombolBeli() {
  ...
  beliButtons.forEach((btn) => {
    ...
    btn.addEventListener("click", async () => {
      ...
      if (productId) {
        fetch(`${API_BASE_URL}/publik/produk/${productId}/view`, { method: "POST" }).catch(() => {});
      }

      if (kontak) {
        const nomor = kontak.replace(/\D/g, "").replace(/^0/, "62");
        window.open(`https://wa.me/${nomor}?text=${encodeURIComponent(`Halo, saya tertarik dengan ${namaProduk}`)}`, "_blank", "noopener");
        return;
      }
