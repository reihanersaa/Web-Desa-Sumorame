...
async function loadGlobalFooter() {
  ...
  let footerEl = document.getElementById("globalFooter");
  
  if (!footerEl) {
    // Jika tag <footer> belum ada id-nya di HTML, buat secara otomatis di bawah body
    footerEl = document.createElement("footer");
    footerEl.id = "globalFooter";
    footerEl.className = "bg-green-900 text-white mt-16 px-6 py-10 opacity-0 translate-y-10 transition-all duration-700";
  ...
  footerEl.innerHTML = `
    <div class="max-w-7xl mx-auto grid md:grid-cols-3 gap-8">
      <div class="footer-item opacity-0 translate-y-6 transition-all duration-700">
      ...
      <div class="footer-item opacity-0 translate-y-6 transition-all duration-700 delay-200">
        <h4 class="kontak-item opacity-0 -translate-y-6 transition-all duration-700 mb-3 font-semibold">
          Kontak
        </h4>
        <p class="kontak-item flex items-start gap-2 opacity-0 -translate-x-10 transition-all duration-700 delay-100 text-sm text-gray-300 mb-2">
        ...
        <p class="kontak-item flex items-center gap-2 opacity-0 translate-y-10 transition-all duration-700 delay-200 text-sm text-gray-300 mb-2">
          <span class="material-symbols-outlined text-lg"> call </span>
          <span id="noTelpFooter">Memuat nomor telepon...</span>
        </p>
        <p class="kontak-item flex items-center gap-2 opacity-0 translate-x-10 transition-all duration-700 delay-300 text-sm text-gray-300">
          <span class="material-symbols-outlined text-lg"> mail </span>
          <span id="emailFooter">Memuat email...</span>
        </p>
      </div>
      <div class="footer-item opacity-0 translate-y-6 transition-all duration-700 delay-300">
